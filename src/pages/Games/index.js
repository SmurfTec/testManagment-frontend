...
const TABLE_HEAD = [
  ...
  { _id: v4(), category: 'category', label: 'category', alignRight: false },
...
export default function Games() {
  const { games, loading, changeGameStatus } = useContext(GamesContext);
  ...
  const handleDelete = (_id) => {
    // * Handle
    console.log(`_id`, _id);
    changeGameStatus(_id, 'rejected');
  };
  const handleApprove = (_id) => {
    // * Handle
    console.log(`_id`, _id);
    changeGameStatus(_id, 'approved');
  };
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - (games?.length || 0)) : 0;
                        ...
                        .map((row) => {
                          ...
                          return (
                            <TableRow
                              ...
                              sx={{ cursor: 'pointer', textDecoration: 'none' }}
                              onClick={() =>
                                navigate(`/dashboard/games/${row._id}`)
                              }
                            >
